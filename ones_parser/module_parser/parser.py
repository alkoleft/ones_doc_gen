...
def parse_module(module_info, file_name):
    with open(file_name, 'r', encoding='utf-8') as stream:
        parse_stream(module_info, stream)
        stream.close()
...
def parse_stream(module_info, stream):
    """
...
def find_methods(text):
    ...
    for match in result:
        ...
        m.parameters = parse_parameters(match.group(7))
        m.parameters_str = match.group(7)
        m.body = match.group(10)

        parse_comment(m, match.group(2))
        yield m


def parse_comment(m, comment):
    """
    ...
    return parse_standard_comment(m, comment) or \
           parse_raw_comment(m, comment)
...
def parse_raw_comment(m, comment):
    m.description = comment
    pass


def parse_standard_comment(m, comment):
